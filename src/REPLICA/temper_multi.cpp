...
/* ----------------------------------------------------------------------
   this is a small modification of REPLICA/temper.cpp to allow multiple
   temperature control fixes to be adjusted during swaps
------------------------------------------------------------------------- */

TemperMulti::TemperMulti(LAMMPS *lmp) : Command(lmp) {}

/* ---------------------------------------------------------------------- */

TemperMulti::~TemperMulti()
{
  MPI_Comm_free(&roots);
...
void TemperMulti::command(int narg, char **arg)
{
  ...
  if ((!utils::strmatch(arg[3],"^nvt")) &&
      (!utils::strmatch(arg[3],"^langevin")) &&
      (!utils::strmatch(arg[3],"^gl[de]$")) &&
      (!utils::strmatch(arg[3],"^rigid/nvt")) &&
      (!utils::strmatch(arg[3],"^temp/")))
    error->universe_all(FLERR,"Tempering temperature fix is not supported");

  auto whichfixes = modify->get_fix_by_style("^" + std::string(arg[3]) + "$");
  int nfixes = whichfixes.size();
  if (nfixes == 0)
    error->universe_all(FLERR,fmt::format("No temperature fixes of style {} found", arg[3]));
  ...
  if (narg == 7) {
    ...
    for (int i = 0; i < nfixes; i++)
      whichfixes[i]->reset_target(new_temp);
  }
...
void TemperMulti::scale_velocities(int t_partner, int t_me)
{
